...
function populateRests(){
  ...
  state.restaurants.forEach(r=>{
    const opt = document.createElement('option'); opt.value=r.id; opt.textContent = r.id;
    sel.appendChild(opt);
  });
}
function populateModalRests(){
  ...
  sel.innerHTML='';
  state.restaurants.forEach(r=>{
    const opt = document.createElement('option'); opt.value=r.id; opt.textContent = ${r.id} (${r.pos.av},${r.pos.ca});
    sel.appendChild(opt);
  });
}
...
function createOrderManual(coords, restId){
  ...
  const cmd = { type:'cmd', cmd:'CREATE_ORDER', house: {av: coords.av, ca: coords.ca}, restaurant: restId };
...
function refreshTables(){
  ...
  for(const o of state.orders){
    ...
    const tdId = document.createElement('td'); tdId.textContent = o.id; tr.appendChild(tdId);
    const tdState = document.createElement('td'); 
    const span = document.createElement('span');
    span.className = 'status-badge ' + statusClass(o.state);
    span.textContent = o.state;
    tdState.appendChild(span); tr.appendChild(tdState);
    const tdPrep = document.createElement('td'); tdPrep.textContent = o.t_prep_s ? ${o.t_prep_s}s : '-'; tr.appendChild(tdPrep);
    const tdDriver = document.createElement('td'); tdDriver.textContent = o.assignedTo || '-'; tr.appendChild(tdDriver);
    tbody.appendChild(tr);
  }

  // drivers
  const tbodyD = document.querySelector('#driversTable tbody'); tbodyD.innerHTML='';
  for(const d of state.drivers){
    const tr = document.createElement('tr');
    const td1 = document.createElement('td'); td1.textContent = d.id; tr.appendChild(td1);
    const td2 = document.createElement('td'); td2.textContent = (d.load && d.load.length) ? d.load.length : 0; tr.appendChild(td2);
    const td3 = document.createElement('td'); td3.textContent = d.target ? ${d.target.av},${d.target.ca} : '-'; tr.appendChild(td3);
    tbodyD.appendChild(tr);
  }

  // restaurants
  const tbodyR = document.querySelector('#restsTable tbody'); tbodyR.innerHTML='';
  for(const r of state.restaurants){
    const tr = document.createElement('tr');
    tr.innerHTML = <td>${r.id}</td><td>${r.pos.av},${r.pos.ca}</td><td>${r.queue ?? 0}</td><td>${r.algo ?? 'SJF'}</td>;
...
function createDefaultMap(){
  ...
  state.houses = [];
  for(let i=0;i<12;i++){
    state.houses.push({id:'H'+(i+1), pos:{av: Math.floor(Math.random()*8)+1, ca: Math.floor(Math.random()*8)+1}});
  }
  state.drivers = [
...
function mockGenerateSnapshot(){
  ...
  for(const o of state.orders){
    if(o.state === 'PREPARING'){
      ...
      if(o.t_left_s <= 0){ o.state = 'READY'; state.history.unshift({ts:Date.now(), msg:Order ${o.id} READY}); }
    }
  }
  ...
  const snap = {
    ...
    orders: state.orders,
    metrics: { avg_wait_s: 0, avg_delivery_s:0, kitchen_algo:document.getElementById('kitchenAlgo').value, driver_policy:'RR+nearest' }
  };
  return snap;
}
...
function mockHandleIncomingCommand(cmd){
  ...
  if(cmd.type === 'cmd' && cmd.cmd === 'CREATE_ORDER'){
    const id = mockOrderId++;
    const houseId = 'H' + (state.houses.length+1);
    // if house coordinates given, find nearest existing house or append
    const coords = cmd.house;
    // create a temporary house representation
    ...
    setTimeout(()=> {
      const o = state.orders.find(x=>x.id===id); if(o) o.state='PREPARING';
    }, 500);
